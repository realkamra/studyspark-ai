...
function GuideCharacter({
  ...
  const messages: Record<DemoMode, string> = {
    notes: "I speak fluent textbook.",
    video: "Tiny movie. Big brain energy.",
  ...
  return (
    <motion.button
      type="button"
      aria-label="Get a study pep talk"
      onClick={onClick}
      ...
      <div className="rounded-[23px] border border-[#17201d]/10 bg-[#f7f8f5] p-4 sm:p-5">
        <div className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div><p className="text-sm font-bold text-[#17201d]">The study glow-up</p><p className="text-[11px] text-[#7a837e]">A very small miracle</p></div>
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="flex min-h-[260px] flex-col rounded-[22px] border border-[#17201d]/10 bg-white p-4">
            <div className="mb-3 flex items-center justify-between"><p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#7a837e]">Your brain dump</p><FileText className="h-4 w-4 text-[#ef5f47]" /></div>
            <textarea
              value={notes}
              onChange={(event) => setNotes(event.target.value)}
              placeholder="Paste lecture notes, a study guide, or the thing you swear you'll read tonight..."
...
const featureCards = [
  {
    ...
    copy: "Dense lecture notes become clean, skimmable explanations with the jargon gently removed.",
    color: "bg-[#d8f36a]",
  },
  {
    icon: Video,
    ...
    copy: "Turn a wall of text into a tiny illustrated lesson your brain will actually remember.",
    color: "bg-[#ff967f]",
  },
  {
    icon: Layers3,
    number: "03",
    title: "Practice without panic",
    copy: "Get flashcards and quick checks that help you find the gaps before the exam does.",
    color: "bg-[#9eb8ff]",
  },
];
    ...
    <main className="min-h-screen overflow-hidden bg-[#f7f8f5] text-[#17201d] selection:bg-[#d8f36a] selection:text-[#17201d]">
      <nav className="relative z-30 mx-auto flex w-full max-w-[1240px] items-center justify-between px-5 py-5 sm:px-8 lg:px-10 lg:py-7" aria-label="Main navigation">
        ...
        <div className="hidden items-center gap-8 text-sm font-semibold text-[#68736c] md:flex">
          <button type="button" onClick={() => scrollToSection("toolkit")} className="transition-colors hover:text-[#17201d]">What it does</button>
          ...
          <button type="button" onClick={() => scrollToSection("stories")} className="transition-colors hover:text-[#17201d]">Student notes</button>
        </div>
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="absolute left-5 right-5 top-[76px] rounded-2xl border border-[#17201d]/10 bg-white p-3 shadow-xl md:hidden">
            <button type="button" onClick={() => { scrollToSection("toolkit"); setMobileOpen(false); }} className="block w-full rounded-lg px-3 py-3 text-left text-sm font-bold hover:bg-[#f7f8f5]">What it does</button>
      ...
      <section id="top" className="relative mx-auto max-w-[1240px] px-5 pb-20 pt-10 sm:px-8 sm:pt-16 lg:px-10 lg:pb-28 lg:pt-20">
        ...
        <div className="relative grid items-center gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:gap-14">
          <motion.div initial={{ opacity: 0, x: -22 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.65 }} className="relative z-10">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#17201d]/10 bg-white px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.1em] text-[#68736c] shadow-sm"><span className="h-2 w-2 rounded-full bg-[#ef5f47]" />For the chronically overwhelmed</div>
            <h1 className="max-w-[650px] text-[clamp(3.25rem,6.2vw,5.8rem)] font-extrabold leading-[0.93] tracking-[-0.07em] text-[#17201d]">Your notes are messy.<br /><span className="relative inline-block">Your study plan <span className="relative z-10">isn&apos;t.</span><span className="absolute bottom-1 left-0 right-0 -z-0 h-3 -rotate-2 rounded-full bg-[#d8f36a] sm:h-4" /></span></h1>
            <p className="mt-7 max-w-[520px] text-base leading-7 text-[#68736c] sm:text-lg">Notefox turns the lecture you half-listened to into clear notes, tiny illustrated lessons, and flashcards that do not feel like punishment.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <button type="button" onClick={goToAuth} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#17201d] px-5 py-3.5 text-sm font-bold text-white shadow-[0_4px_0_#0c100e] transition-all hover:-translate-y-0.5 hover:bg-[#26362d]">Start studying for free <ArrowRight className="h-4 w-4" /></button>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-x-5 gap-y-3 text-xs font-semibold text-[#87908a]"><span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#ef5f47]" /> No credit card</span><span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#ef5f47]" /> Built for busy brains</span><span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#ef5f47]" /> Actually understandable</span></div>
          </motion.div>
          <div id="demo" className="relative z-10 scroll-mt-8"><StudyDemo /></div>
        </div>
      </section>

      <section className="border-y border-[#17201d]/10 bg-white" aria-label="Student proof">
        <div className="mx-auto flex max-w-[1240px] flex-wrap items-center justify-between gap-x-8 gap-y-4 px-5 py-5 sm:px-8 lg:px-10">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#98a09b]">A better way to survive</p>
          <div className="flex flex-wrap items-center gap-x-7 gap-y-3 text-sm font-bold text-[#68736c]"><span className="flex items-center gap-2"><GraduationCap className="h-4 w-4 text-[#ef5f47]" /> 12,000+ study sessions</span><span className="flex items-center gap-2"><MessageCircle className="h-4 w-4 text-[#4e7bff]" /> 4.9/5 from tired students</span><span className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-[#d1ad00]" /> 100% less textbook fog</span></div>
        </div>
      </section>

      <section id="toolkit" className="mx-auto max-w-[1240px] scroll-mt-8 px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        <div className="mb-10 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-[#ef5f47]">The study glow-up</p><h2 className="max-w-[620px] text-4xl font-extrabold leading-[0.98] tracking-[-0.055em] sm:text-5xl">From “what is this?”<br />to “ohhh, got it.”</h2></div><p className="max-w-[300px] text-sm leading-6 text-[#68736c]">One place for the stuff you need to learn, remember, and explain without staring into the middle distance.</p></div>
      ...
      <section id="how-it-works" className="scroll-mt-8 bg-[#17201d] px-5 py-20 text-white sm:px-8 lg:px-10 lg:py-24">
        <div className="mx-auto grid max-w-[1240px] gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-[#d8f36a]">No productivity cosplay</p><h2 className="max-w-[450px] text-4xl font-extrabold leading-[0.98] tracking-[-0.055em] sm:text-5xl">Three steps between you and the “I got this.”</h2><p className="mt-6 max-w-[390px] text-sm leading-6 text-white/60">Bring the chaos. Notefox handles the organizing, simplifying, and gentle academic intervention.</p><button type="button" onClick={goToAuth} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-[#d8f36a] px-5 py-3.5 text-sm font-bold text-[#17201d] transition-transform hover:-translate-y-0.5">Try Notefox <ArrowUpRight className="h-4 w-4" /></button></div><div className="grid gap-3">{[{ icon: BookOpen, title: "Drop in the chaos", copy: "Paste notes, upload a study guide, or start with a topic." }, { icon: WandSparkles, title: "Pick your brain food", copy: "Choose a clear guide, an illustrated mini lesson, or flashcards." }, { icon: CircleCheck, title: "Actually remember it", copy: "Review at your pace with quick checks that show what stuck." }].map(({ icon: Icon, title, copy }, index) => (<div key={title} className="flex items-start gap-4 rounded-[20px] border border-white/10 bg-white/[0.04] p-5"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-[#17201d]"><Icon className="h-5 w-5" /></div><div><div className="flex items-center gap-2"><span className="text-xs font-bold text-[#d8f36a]">0{index + 1}</span><h3 className="font-bold">{title}</h3></div><p className="mt-1 text-sm leading-6 text-white/55">{copy}</p></div></div>))}</div></div>
      </section>

      <section id="stories" className="mx-auto max-w-[1240px] scroll-mt-8 px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]"><div className="rounded-[26px] bg-[#ff967f] p-7 sm:p-10"><div className="flex items-start justify-between gap-5"><p className="max-w-[500px] text-2xl font-extrabold leading-[1.02] tracking-[-0.04em] sm:text-4xl">“It turned three hours of lecture notes into something I could explain to my roommate in five minutes.”</p><span className="text-3xl">“</span></div><div className="mt-12 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#17201d] text-sm font-bold text-white">JM</div><div><p className="text-sm font-bold">Jordan M.</p><p className="text-xs text-[#17201d]/60">Neuroscience major · UC Davis</p></div></div></div><div className="flex flex-col justify-between rounded-[26px] border border-[#17201d]/10 bg-white p-7 sm:p-10"><div><div className="mb-7 flex h-11 w-11 items-center justify-center rounded-[14px] bg-[#9eb8ff]"><Sparkles className="h-5 w-5" /></div><h3 className="max-w-[280px] text-2xl font-extrabold leading-[1.05] tracking-[-0.04em]">Your future self has a quiz on Friday.</h3><p className="mt-3 text-sm leading-6 text-[#68736c]">Do them a solid. Start with the notes you already have.</p></div><button type="button" onClick={goToAuth} className="mt-8 flex items-center justify-between border-t border-[#17201d]/10 pt-5 text-sm font-bold">Make future-you proud <ArrowRight className="h-4 w-4 text-[#ef5f47]" /></button></div></div></section>

      <section className="px-5 pb-10 sm:px-8 lg:px-10"><div className="relative mx-auto max-w-[1240px] overflow-hidden rounded-[28px] bg-[#d8f36a] px-6 py-12 sm:px-12 lg:py-16"><span className="absolute -right-14 -top-20 h-48 w-48 rounded-full border-[32px] border-[#17201d]/10" /><span className="absolute bottom-[-100px] left-[42%] h-44 w-44 rounded-full border-[24px] border-[#ef5f47]/20" /><div className="relative flex flex-col justify-between gap-8 md:flex-row md:items-end"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.14em] text-[#17201d]/55">Your notes called</p><h2 className="max-w-[650px] text-4xl font-extrabold leading-[0.95] tracking-[-0.055em] sm:text-5xl">Let&apos;s make studying<br />a little less dramatic.</h2></div><button type="button" onClick={goToAuth} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#17201d] px-5 py-3.5 text-sm font-bold text-white shadow-[0_4px_0_#0c100e] transition-transform hover:-translate-y-0.5">Create your free space <ArrowRight className="h-4 w-4" /></button></div></div></section>
