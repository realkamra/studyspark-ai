...
/**
 * SKYBOUND — Meadow Isles (Standalone harness)
 * Demo build for /#/demo/skybound that runs without a Convex provider.
 * All Convex persistence calls are no-ops. Game logic is identical to
 * SkyboundRunner; content is injected via props.
 */
...
export default function SkyboundStandalone({
  quiz,
  gamePairs,
}: {
  quiz: QuizQuestion[];
  ...
  const recordRun = useCallback(async (_args: unknown) => {}, []);
  const bestPastScore = 0;
      ...
      for (const o of obstaclesRef.current) {
        ...
        if (hit) {
          ...
          if (livesRef.current <= 0) {
            ...
            void recordRun({ materialId: undefined as unknown as Id<"materials">, gameType: "speed", score: Math.floor(scoreRef.current), bestStreak: bestComboRef.current });
          } else {
  ...
  }, [recordRun, render, setPhaseBoth, spawnPattern, nextId]);
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-xl border px-3.5 py-2.5 text-left text-sm font-semibold transition
                          ${!showState ? "border-slate-300 bg-white hover:border-violet-400 hover:bg-violet-50 text-slate-800"
